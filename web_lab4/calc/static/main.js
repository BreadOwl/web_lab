...
function clickHandler(event) {
    ...
    for (let i = 0; i < digitsList.length; i++) {
        if (elem == digitsList[i].innerHTML) {
            board.append(elem);
        }
    }

    for (let i = 0; i < operationList.length; i++) {
        if (elem == operationList[i].innerHTML) {
            board.append(elem);
        }
    }

    for (let i = 0; i < bracketList.length; i++) {
        if (elem == bracketList[i].innerHTML) {
            board.append(elem);
        }
    }
...
window.onload = function () {
    ...
    digits.addEventListener('click', clickHandler);
    let other = document.querySelector('.other');
    other.addEventListener('click', clickHandler);
}
